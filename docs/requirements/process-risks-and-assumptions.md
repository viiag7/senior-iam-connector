...
### Diretriz aprovada para o MVP

> O Senior IAM Connector cria/reutiliza a conta assim que o colaborador elegível aparecer na Senior. A habilitação ocorre em D-1 (`hireDate - 1 dia`). Se o cadastro chegar depois desse marco, a conta é criada e habilitada no próximo processamento seguro, desde que o vínculo continue elegível. Quickin permanece fora do escopo.

---

## Premissa validada — não há múltiplos vínculos simultâneos

Gente & Gestão confirmou que o cenário desta organização **não possui múltiplos vínculos simultâneos para a mesma pessoa** no escopo da integração.

Validação funcional: **Juliana Croda — Gente & Gestão**.

Consequências para o desenho:

- o conector pode assumir no máximo um vínculo ativo relevante por CPF por vez;
- não é necessária regra de precedência entre vínculos simultâneos;
- matrícula, cargo, departamento, gestor, empresa e centro de custo do vínculo corrente podem ser projetados diretamente;
- Rehire e mudanças de vínculo ao longo do tempo continuam reutilizando a mesma identidade pelo CPF;
- se essa premissa mudar no futuro, a arquitetura deverá ser revisada antes de aceitar o novo cenário.
